    ...
    def calculate_fmax(self):
        ...
        for i in range(101):
            ...
            if f!=None and f>=fmax:
                fmax = numpy.around(f,decimals=2)
                f_thres = numpy.around(i*0.01,decimals=2)
        self.opt = numpy.around(fmax, decimals=5)
        print(f"JB DEBUG opt: {self.opt}")
        self.thres = f_thres
        print(f"JB DEBUG thresh: {self.thres}")
        print('fmax is %s\n' % fmax)
        print('thres is %s\n' % f_thres)
            ...
            for line in f:
                if line.startswith(onto):
                    if line.split('|')[0].split()[1]==Type and line.split('|')[0].split()[2]==mode:
                        ...
                        break
        print(f"JB: {nfmax}, {self.opt}, {thres}, {self.thres}")
            ...
            for line in f:
                if line.startswith(onto):
                    if line.split('|')[0].split()[1]==Type and line.split('|')[0].split()[2]==mode:
                        ...
                        print(f"JB DEBUG COVERAGE: {coverage}")
                        coverage = float(coverage)
                        self.coverage=str(numpy.around(coverage,decimals=2))
                        print(f"JB DEBUG: {self.coverage}")
                        break
...
def getprrc(onto,Type,mode,prrcfolder,method, results_folder):
    ...
    mymethod = method
    if'/' in method:
        mymethod = method.split('/')[-1]
    filename = os.path.join(prrcfolder,f'{mymethod}_prrc.txt')
    # print(f"125 SEEKING {filename} in {prrcfolder}")
    r = result()
    r.read_info(onto,Type,mode,mymethod)
        ...
        for line in f:
            if line.startswith('>'):
                ...
                if fields[0]==onto and fields[1]==Type and fields[2]==mode:
                    ...
                    nextline = f.readline()
                    # print(nextline)
                    pr = nextline.split('|')[1].strip().split()
                    # print(pr)
                    rc = f.readline().strip().split()
                    # print(rc)
                    r.read_prrc(pr,rc)
                    r.calculate_fmax()
                    print("JB: After Calculate Fmax")
                    if r.check_fmax(onto,Type,mode,method, results_folder):
                        print("JB: Getting Coverage")
...
def read_config():
    ...
    try:
        config_dict = yaml.load(args.config_stream, Loader=yaml.FullLoader)['plot']
    ...
    if os.path.isdir(results_folder):
        ...
        if os.path.isdir(prrc_folder):
            ...
            for method in methods:
                ...
                # print(f"256 {method}")
                # print(f"257 {prrc_folder}")
                file_prrc = f"{prrc_folder}{method}_prrc.txt"
                # print(f"259 Seeking {file_prrc} in {prrc_folder}")
...
if __name__=='__main__':
    ...
    if check_existence(results_folder, methods):
        ...
        # print(f"JB: RESULTS FOLDER {prrcfolder}")
